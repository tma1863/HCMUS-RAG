...
import hashlib
import time
from config import config  # Import config instance từ config.py
from .simple_embedding_model import SimpleEmbeddingModel  # Import SimpleEmbeddingModel

logger = logging.getLogger(__name__)

def compute_mdhash_id(content: str, prefix: str = "") -> str:
    """
    Tính toán ID hash chuẩn hóa cho nội dung.
    
    Args:
        content: Nội dung cần tạo hash
        prefix: Tiền tố để phân biệt các loại hash khác nhau
        
    Returns:
        Chuỗi hash với tiền tố
    """
    content_str = str(content).encode('utf-8')
    return f"{prefix}{hashlib.md5(content_str).hexdigest()}"
...
class EmbeddingStore:
    def __init__(self, embedding_model, db_filename, batch_size, namespace):
        ...
        Khởi tạo giống CHÍNH XÁC với HippoRAG gốc.
    ...
    def get_passage_count(self):
        """Get the number of passages/records in the store"""
        return len(self.hash_ids)

    def get_entity_count(self):
        """Get the number of entities/records in the store"""
        return len(self.hash_ids)

    def get_fact_count(self):
        """Get the number of facts/records in the store"""
        return len(self.hash_ids)